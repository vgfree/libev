...
function (or via the C<ev_default_loop (0)> macro).
...
more often than 100 times per second:
   ...
   ev_set_timeout_collect_interval (ev_default_loop_uc_ (), 0.1);
   ev_set_io_collect_interval (ev_default_loop_uc_ (), 0.01);
...
=item ev_set_invoke_pending_cb (loop, void (*invoke_pending_cb)(struct ev_loop *loop))
...
=item ev_set_loop_release_cb (loop, void (*release)(struct ev_loop *loop) throw (), void (*acquire)(struct ev_loop *loop) throw ())
...
whole section.
   ...
   ev_io_start (ev_default_loop_uc_ (), &w);
...
other events are pending:
   ...
   idle_cb (struct ev_loop *loop, ev_idle *w, int revents)
   {
     ...
     ev_io_start (struct ev_loop *loop, &io);
   }
...
within the callback:
   ...
   static void
   callback (struct ev_loop *loop, ev_timer *w, int revents)
   {
     // calculate when the timeout would happen
     ev_tstamp after = last_activity - ev_now (loop) + timeout;
...
its completion.
   ...
   static void
   child_cb (struct ev_loop *loop, ev_child *w, int revents)
   {
   ...
   if (pid < 0)
   ...
   else
     {
       ...
       ev_child_start (ev_default_loop (0), &cw);
     }
...
C<ev_timer> callback invocation).
   ...
   static void
   stat_cb (struct ev_loop *loop, ev_stat *w, int revents)
   {
...
libglib event loop.
   ...
   event_poll_func (GPollFD *fds, guint nfds, gint timeout)
   {
     ...
     // stop timer again
     if (timeout >= 0)
       ev_timer_stop (loop, &to);

     // stop io watchers again - their callbacks should have set
     for (n = 0; n < nfds; ++n)
       ev_io_stop (loop, iow [n]);
...
C<ev_loop_destroy (ev_default_loop (0))> followed by C<ev_default_loop (...)>.
...
an example that does that for some fictitious SIGUSR1 handler:
   ...
   sigusr1_handler (void)
   {
     ...
     // no locking etc.
     queue_put (data);
     ev_async_send (ev_default_loop (0), &mysig);
   }

   static void
   mysig_cb (struct ev_loop *loop, ev_async *w, int revents)
   {
     sometype data;
     sigset_t block, prev;
...
real programmers):
   ...
   static void
   t1_cb (struct ev_loop *loop, ev_timer *w, int revents)
   {
   ...
   static void
   t2_cb (struct ev_loop *loop, ev_timer *w, int revents)
   {
     struct my_biggy big = (struct my_biggy *)
...
triggered, using C<EVRUN_ONCE>:
   ...
   while (!exit_main_loop)
     ev_run (ev_default_loop (0), EVRUN_ONCE);
...
First, you need to associate some data with the event loop:
   ...
   void prepare_loop (struct ev_loop *loop)
   {
      ...
      pthread_create (&u->tid, 0, l_run, loop);
   }
...
that might have been added:
   ...
   static void
   async_cb (struct ev_loop *loop, ev_async *w, int revents)
   {
...
thread to continue:
   ...
   static void
   real_invoke_pending (struct ev_loop *loop)
   {
     userdata *u = ev_userdata (loop);
...
with. If it is omitted, it will use C<ev_default_loop (0)>.
...
loop argument"). The C<loop> form is used when this is the sole argument,
C<loop,> is used when other arguments are following. Example:

   ev_unref (loop);
   ev_timer_add (loop, watcher);
   ev_run (loop, 0);

It assumes the variable C<loop> of type C<struct ev_loop *> is in scope,
which is often provided by the following macro.

=item C<struct ev_loop *loop>, C<struct ev_loop *loop,>

This provides the loop I<parameter> for functions, if one is required ("ev
loop parameter"). The C<struct ev_loop *loop> form is used when this is the sole parameter,
C<struct ev_loop *loop,> is used when other parameters are following. Example:

   // this is how ev_unref is being declared
   static void ev_unref (struct ev_loop *loop);

   // this is how you can declare your typical callback
   static void cb (struct ev_loop *loop, ev_timer *w, int revents)

It declares a parameter C<loop> of type C<struct ev_loop *>, quite
suitable for use with C<loop>.

=item C<ev_default_loop (0)>, C<ev_default_loop (0),>
...
=item C<ev_default_loop_uc_ ()>, C<ev_default_loop_uc_ (),>

Usage identical to C<ev_default_loop (0)> and C<ev_default_loop (0),>, but requires that the
default loop has been initialised (C<UC> == unchecked). Their behaviour
is undefined when the default loop has not been initialised by a previous
execution of C<ev_default_loop (0)>, C<ev_default_loop (0),> or C<ev_default_init (...)>.

It is often prudent to use C<ev_default_loop (0)> when initialising the first
watcher in a function but use C<ev_default_loop_uc_ ()> afterwards.
...
or not.
   ...
   static void
   check_cb (struct ev_loop *loop, ev_timer *w, int revents)
   {
   ...
   ev_check_start (ev_default_loop (0), &check);
...
Note that C<ev_default_loop (0)> and C<ev_default_loop (0),> will no longer provide a
